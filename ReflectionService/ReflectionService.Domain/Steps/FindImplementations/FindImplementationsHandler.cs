...
public sealed class FindImplementationsHandler : HandlerTemplateBase<FindImplementationsArgs>
{
    public FindImplementationsHandler() : base("FindImplementations") { }

    internal protected override TypesResult StartCheck(CheckingContext context, ManifestStep step, FindImplementationsArgs args)
    {
        ...
        var impls = GetAllTypesSafe(context.UserAssembly)
            .Where(t => t.IsClass)
            .Where(t => args.IncludeAbstract || !t.IsAbstract)
            .Where(t => MatchVisibility(t, args.Visibility))
            .Where(t => !excludeCg || !IsCompilerGenerated(t))
            .Where(t => abstractions.Any(abs => abs.IsAssignableFrom(t)))
            .OrderBy(t => t.FullName)
            .ToArray();

        return new TypesResult(impls);
    }
    ...
    internal protected override void WriteResult(CheckingContext context, ManifestStep step, TypesResult results)
    {
        ...
        {
            context.StepResults.Add(new(step.Id, step.Operation, false, FailureSeverity.Error, "Реализации не найдены"));
            return;
        }
